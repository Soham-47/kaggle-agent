...
def test_generation_store_snapshot_includes_uncommitted_source(tmp_path: Path):
    import subprocess
    ...
    run = lambda *args: subprocess.run(("git", "-C", str(root), *args), check=True, capture_output=True)
    run("init", "-q")
    (root / "pipeline.py").write_text("RECIPE = 1\n", encoding="utf-8")
    run("add", "pipeline.py")
    run("-c", "user.email=t@e", "-c", "user.name=t", "commit", "-qm", "init")
    (root / "pipeline.py").write_text("RECIPE = 2\n", encoding="utf-8")
    state = SupervisorStateStore(RuntimeLayout.for_repo(root, tmp_path / "state"))
    generation = GenerationStore(state).create_snapshot(root)
    assert Path(generation.path).joinpath("pipeline.py").read_text() == "RECIPE = 2\n"
    assert generation.path != str(root)
    assert generation.revision.tree_sha != read_tree_revision(root)
