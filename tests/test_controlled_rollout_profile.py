...
def test_controlled_auto_safe_profile_is_explicit_and_restricted():
    ...
    assert repair["max_repairs_per_cycle"] == 1
    assert repair["max_attempts_per_incident"] == 2
    assert repair["max_repairs_per_day"] == 2
    assert repair["max_changed_source_files"] == 2
    assert repair["max_changed_test_files"] == 1
    assert repair["max_changed_lines"] == 120
...
def test_controlled_profile_is_explicitly_loadable_without_changing_defaults():
    ...
    assert supervisor.repair.max_repairs_per_cycle == 1
