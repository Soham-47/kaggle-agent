...
- labels:
  ...
  - `train.csv` has `Report`; `test.csv` does not — **text is not an inference input**. Gold table appears only as `gold = …[TARGETS]` for holdout checks (rows with all labels present).
