...
|------|---------------|-----------|--------|
| Multilingual **report → weak labels** on ~4,349 unlabeled studies | Duvanov: almost all supervision is in reports; 58 gold rows are the exception | **Must implement** for train. Hidden **test reports are not confirmed** — do not depend on report text at infer | Extractor + train on weak labels |
