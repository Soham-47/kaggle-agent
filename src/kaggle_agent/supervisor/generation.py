...
class GenerationStore:
    ...
    def create_snapshot(
        ...
        *,
        ...
        """Materialize the current source tree, including uncommitted CODE output.

        ``git worktree add HEAD`` is intentionally limited to committed files,
        which can silently drop a generated recipe.  A snapshot gives each
        worker a stable path and content digest while leaving mutable datasets
        and runtime state outside the generation.
        """
        generation_id = self.new_id()
        ...
        def ignore(_path: str, names: list[str]) -> set[str]:
            return {
                name
                for name in names
                if name in {".git", ".agent", "__pycache__", "data", "notebooks", "submissions", "logs"}
                or name == ".env"
                or name.startswith(".env.")
                or name.lower() in {"credentials", "credentials.json", "credentials.yaml", "credentials.yml", "kaggle.json", "service-account.json", ".aws", ".kaggle", ".ssh", ".netrc", ".npmrc", ".pypirc", "id_rsa", "id_ed25519"}
                or fnmatch.fnmatch(name.lower(), "*.pem")
                or fnmatch.fnmatch(name.lower(), "*credential*")
            }

        shutil.copytree(source_root, destination, ignore=ignore)
        revision = RuntimeRevision(
            read_git_revision(source_root), read_tree_revision(destination), generation_id
        )
        generation = RuntimeGeneration(
            generation_id=generation_id,
            revision=revision,
