...
class SupervisorRecovery:
    ...
    def inspect_worker(self, worker_id: str, *, timeout_seconds: float) -> WorkerRecovery:
        ...
        if alive and owner and fresh:
            action = "ADOPT"
        elif alive:
            action = "TERMINATE_OR_RECONCILE"
        else:
            action = "START_OR_RESUME"
        return WorkerRecovery(worker_id, pid if isinstance(pid, int) else None, bool(owner), fresh, action)
