...
class Heartbeat:
    ...
    timestamp: float
    # Older heartbeat records have no progress timestamp. Treat their last
    # liveness beat as the most recent progress so reload remains safe.
    progress_timestamp: float | None = None

    def __post_init__(self) -> None:
        if self.progress_timestamp is None:
            object.__setattr__(self, "progress_timestamp", self.timestamp)
...
class HeartbeatStore:
    ...
    def is_fresh(self, worker_id: str, *, timeout_seconds: float, now: float | None = None) -> bool:
        ...
        return heartbeat is not None and 0 <= current - heartbeat.timestamp < timeout_seconds
...
class HeartbeatThread:
    ...
    def update(self, *, stage: str | None = None, progress: str | None = None) -> None:
        """Persist meaningful progress immediately, alongside liveness beats."""
        with self._lock:
            ...
            self.heartbeat = Heartbeat(
                **{
                    **asdict(current),
                    ...
                    "last_progress_event": progress if progress is not None else current.last_progress_event,
                    "timestamp": now,
            ...
            with self._lock:
                ...
                self.heartbeat = Heartbeat(
                    **{
                        **asdict(current),
                        "timestamp": time.time(),
                        "progress_timestamp": current.progress_timestamp,
                    }
                )
                self.store.write(self.heartbeat)
